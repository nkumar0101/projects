...
void fsutil_extract(char** argv UNUSED) {
  ...
  for (;;) {
    ...
    // read_cache(sector++, header, 0, BLOCK_SECTOR_SIZE);
    ...
    if (type == USTAR_EOF) {
      ...
      while (size > 0) {
        ...
        // read_cache(sector++, data, 0, BLOCK_SECTOR_SIZE);
  ...
  // write_cache(0, header, 0, BLOCK_SECTOR_SIZE);
  // write_cache(1, header, 1, BLOCK_SECTOR_SIZE);
...
void fsutil_append(char** argv) {
  ...
  // write_cache(sector++, buffer, 0, BLOCK_SECTOR_SIZE);
  ...
  // write_cache(sector, buffer, 0, BLOCK_SECTOR_SIZE);
  // write_cache(sector + 1, buffer, 0, BLOCK_SECTOR_SIZE);
